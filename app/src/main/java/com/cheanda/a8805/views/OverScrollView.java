...
public class OverScrollView extends ScrollView {
	...
	private OnIsCanPullDown mOnIsCanPullDown;
	 ...
	 * 滚动最大高度,超过此高度不再滚动
	 */
	private static final int OVERSCROLL_MAX_HEIGHT = 500;
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (isRecord) {
				...
				if(Math.abs(overScrollDistance) >= OVERSCROLL_MAX_HEIGHT && overScrollDistance * mDeltaY > 0){
					mDeltaY = 0;
				}

				//如果滚动到ScrollView自身滚动边界，直接调用自身滚动
				if(overScrollDistance *(overScrollDistance + mDeltaY) < 0){
				...
				if(overScrollDistance * mDeltaY > 0){
					mDeltaY = (int) (mDeltaY * ELASTICITY_COEFFICIENT);
				}
				
				if(overScrollDistance == 0){
					mDeltaY = (int) (mDeltaY * ELASTICITY_COEFFICIENT * 0.5f);
				}
				
				if(overScrollDistance == 0 && mDeltaY == 0){
					break;
				}

				//检测最终滚动距离，最大为20
				if(Math.abs(mDeltaY) > 20){
					mDeltaY = mDeltaY > 0 ? 20 : -20;
				}

				// 记录滚动总距离
				overScrollDistance += mDeltaY;
				...
				// 滚动视图
				mContentLayout.smoothScrollBy(0, mDeltaY);
				
				if(mOverScrollTinyListener != null){
					mOverScrollTinyListener.scrollDistance(mDeltaY, overScrollDistance);
				}
				return true;
			}
			break;
...
//		Log.v("test", "deltaY "+deltaY+"   scrollY "+scrollY);
	...
	public void setOnIsCanPullDown(OnIsCanPullDown o){
		mOnIsCanPullDown = o;
	}
	...
	public interface OnIsCanPullDown{
		void isPullDwon(boolean b);
	}
