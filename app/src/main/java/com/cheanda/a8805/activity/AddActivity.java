...
public class AddActivity extends BaseActivity implements View.OnTouchListener {
    ...
    @Bind(R.id.addingge)
    ImageView mAddingge;
    @Bind(R.id.error_image_new)
    ImageView mErrorImageNew;
    @Bind(R.id.error_image_old)
    ImageView mErrorImageOld;
    @Bind(R.id.activity_add)
    RelativeLayout mActivityAdd;
    @Bind(R.id.top)
    ImageView mTop;
            ...
            case 2:
                ...
                if (isStart) {
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(AddActivity.this, "运行失败", Toast.LENGTH_SHORT).show();
                        }
                    });
                }
                break;
            case 3:
                ...
                if (isStart) {
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(AddActivity.this, "停止失败", Toast.LENGTH_SHORT).show();
                        }
                    });
                }
                break;
    ...
    private void checkData() {
        ...
        //验证数据是失败或成功
        if (arr[1] == 16) {
            //42 16 91 95 21 32 00 -11 60 50 10 00 -87 35
            //新油重量参数
            ...
            //已经加主了
            int i = (initNewOilWeight - mNewOilKG) / 10;
            if (i > 0) {
    ...
    private void initListener() {
        ...
        mStart.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (initNewOilWeight > 0) {
                    isStop = true;
                    distinguishResult = 2;
                    //点击开始按钮发送以换油升数位参数的指令
                    //2a 06 02 10 3d 23
                    int i1 = count;
                    int result = 0x2a ^ 6 ^ 2 ^ i1;
                    int[] bytes1 = {0x2a, 0x06, 0x02, i1, result, 0x23};
    ...
    private void initData() {
        mBitmap = BItmapUtil.compressBItmap(this, R.mipmap.bg_img_normal_12);
        mImageView.setImageBitmap(mBitmap);
        mBitmapTop = BItmapUtil.compressBItmap(this, R.mipmap.bg_img_normal_12_1);
        mTop.setImageBitmap(mBitmapTop);
        mStartImage = BItmapUtil.compressBItmap(this, R.mipmap.btn_img_popovers_3);
        mStarting.setImageBitmap(mStartImage);
        mStopImage = BItmapUtil.compressBItmap(this, R.mipmap.text_img_3);
